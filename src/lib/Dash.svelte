...
// Support, contribute or at least follow & engage on X, Linkedin if you use this Dashboard.
// S5 Runes active
...
let  Active_Itm =$derived( 
                ...
                : 'Doc Body - tap on interactive items') 
);
...
>
    <!--header  class ='grid     col-span-full    ' -->
        <Header  fixed  {Active_Itm}  
                class = 'portrait:fixed   w-full  items-center justify-stretch border-x-2 border-muted '
        /> 
        <!--PROPS:  fixed (stays, no shift/scroll)  
    ...
    >       
            <Content  cfg={{Pshow, Right}} />  <!--props: stating space for sidebar or not-->
    </main>
