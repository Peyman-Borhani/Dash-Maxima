...
//import    Fullscreen          from  '~icons/radix-icons/enter-full-screen'
  ...
  <!--class = "border-muted {Right? 'pr-10 ' :'pl-10 '} "-->
<span   class = "{fixed? 'fixed' :'portrait:fixed absolute'} w-full h-fit  m-0 p-0 {!S_bar? 'px-[1.2ch]' :Right? 'pr-[3.2ch]'  :'pl-[3.2ch]'} 
            inline-grid  grid-cols-12  top-0 z-50  min-h-[7.4ch] portrait:min-h-[8ch]  border-b-2  sm:border-b  border-black
            items-center justify-stretch  place-items-center  place-content-evenly  dark:border-gray-400/70
            grid-flow-col  col-span-full  transition-all  duration-1000  bg-gray-300/60  dark:bg-[#161819] 
            gap-x-2  shadow-lg   hover:shadow-muted-foreground  shadow-stone-500/80 "
  >
  ...
  >
    <Breadcrumb.List  class= 'text-md  landscape:text-lg ' >
      <Breadcrumb.Item class='portrait:hidden '>Dashboard</Breadcrumb.Item>
      ...
      <Breadcrumb.Separator   class={On_Page!=='Orders' || !itm? 'hidden ' :'portrait:hidden '}/>
      <Breadcrumb.Item    class={On_Page==='Orders'? '' :'portrait:hidden '}>
        <Breadcrumb.Page  href='#{itm}'> {On_Page==='Orders'? 'Recent Orders'  :On_Page} </Breadcrumb.Page>
      </Breadcrumb.Item>
